        ...
        if host_whois_check:
            ...
            for keyword in self.dm_keywords:
                if keyword.lower() in hostReginfo.lower():
                    in_keyword = True
                    break
            # Check privacy protection (system whois may return different privacy protection identifiers)
            if not in_keyword and ('REDACTED FOR PRIVACY' in hostReginfo or 
                                   'Privacy Protection' in hostReginfo or 
                                   'Whois Privacy' in hostReginfo or
                                   'PRIVACY PROTECTION' in hostReginfo):
                self.whoisNone_rdm.add(root_domain)
...
def test():
    ...
    target_domain = ["https://www.alibaba.com"]
    dm_keywords = ['zhe jiang', 'zhejiang', 'alibaba', 'aliyun']

    root_domain = 'tradesns.com'
    root_domains = []
    hostwhois_dmnums = dict()
    whoisNone_rdm = set()

    host_whois_check = True
    in_keyword = False
    try:
        if root_domain in root_domains:
            in_keyword = True  # If root domain exists, it means this domain belongs to the tested service provider
            host_whois_check = False  # Already determined to belong to tested service provider, no need for another hostwhois request
        else:
            if root_domain in hostwhois_dmnums:  # For other root domains, need to perform hostwhois query to determine if this root domain should be included
                if hostwhois_dmnums[root_domain] > 10:  # Maximum 10 hostwhois requests to ensure accuracy
                    host_whois_check = False
                else:
                    hostwhois_dmnums[root_domain] += 1
            else:
                hostwhois_dmnums[root_domain] = 1
    finally:
        pass
